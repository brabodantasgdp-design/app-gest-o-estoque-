  ...
  async update(id: string, updates: Partial<Tenant>) {
    const { data, error } = await supabase
      ...
      .update({
        ...
        access_days_remaining: updates.accessDaysRemaining,
        expiration_date: updates.expirationDate,
        max_monthly_scans: updates.maxMonthlyScans,
        scans_used_this_month: updates.scansUsedThisMonth,
      })
      .eq('id', id)
      .select()
      .single();
  ...
  async update(id: string, updates: Partial<Product>) {
    const { data, error } = await supabase
      ...
      .update({
        ...
        sale_discount_percent: updates.saleDiscountPercent,
        new_price: updates.newPrice,
        items_sold: updates.itemsSold,
        ficha_tecnica_id: updates.fichaTecnicaId,
        status: updates.status,
      })
      .eq('id', id)
      .select()
...
function mapTenant(row: any): Tenant {
  return {
    ...
    cnpjStore: row.cnpj_store,
    plan: row.plan,
    status: row.status,
    accessDaysRemaining: row.access_days_remaining,
    expirationDate: row.expiration_date,
    maxMonthlyScans: row.max_monthly_scans,
    scansUsedThisMonth: row.scans_used_this_month,
...
function mapFicha(row: any): FichaTecnica {
  const ingredients = (row.recipe_ingredients || []).map((ri: any) => ({
    ...
    insumoName: ri.insumo_name,
    quantity: ri.quantity,
    unit: ri.unit,
    calculatedCost: ri.calculated_cost,
  }));
  ...
  return {
    ...
    ingredients,
    rawInsumoCost: row.raw_insumo_cost,
    wasteMarginPercent: row.waste_margin_percent,
    operationalOverheadPercent: row.operational_overhead_percent,
    taxPercent: row.tax_percent,
    totalProductionCost: row.total_production_cost,
    targetProfitMarginPercent: row.target_profit_margin_percent,
    calculatedPrice: row.calculated_price,
    finalPrice: row.final_price,
    netProfitPerUnit: row.net_profit_per_unit,
    profitMarginRate: row.profit_margin_rate,
  };
}
...
function mapInvoice(row: any): InvoiceScan {
  const items = (row.invoice_items || []).map((ii: any) => ({
    rawName: ii.raw_name,
    matchedInsumoName: ii.matched_insumo_name,
    quantity: ii.quantity,
    unit: ii.unit,
    unitCost: ii.unit_cost,
    totalCost: ii.total_cost,
